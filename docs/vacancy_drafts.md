...
* [создание черновика](#draft_create)
* [редактирование черновика](#draft_update)
...
<a name="draft_create"></a>
## Создание черновика 
```POST /vacancies/drafts```

Запрос для создания черновика вакансий для текущего менеджера. 
Обязательно передавать имя черновика

Формат данных аналогичен [просмотру вакансии](vacancies.md#ответ) кроме id и с условием, что все поля не обязательны. 

Каждое поле будет описано

* `400 Bad Request` – ошибки в полях при добавлении черновика вакансии. 
* `403 Forbidden` – добавление вакансий недоступно данному пользователю.


<a name="draft_update"></a>
## Редактирование черновика
```PUT /vacancies/drafts/{draft_id}```
...
[причины ошибки](errors.md#vacancy_draft).
